...
@Component({
  ...
  imports: [ButtonModule, CardModule, TagModule, SkeletonModule, ChartModule],
...
export class DashboardComponent {
  ...
  protected readonly trafficSummary = signal({
    requestsToday: 182,
    requestsWeek: 1240,
    errorsToday: 4,
    successRate: 99.1
  ...
  protected readonly requestSpark = signal<number[]>([42, 64, 58, 72, 68, 86, 94, 120, 104, 92, 88, 96]);
  protected readonly hourlyRequests = signal<number[]>([18, 24, 32, 40, 38, 52, 64, 72, 90, 82, 76, 68]);
  protected readonly hourlyToolCalls = signal<number[]>([4, 6, 9, 12, 10, 14, 18, 20, 24, 22, 18, 16]);
  protected readonly modelShare = signal([
  ...
  protected readonly latency = signal([
    { label: 'p50', value: 820 },
    { label: 'p90', value: 1340 },
    { label: 'p95', value: 1760 },
    { label: 'max', value: 2240 }
  ]);
  protected readonly uptimePercent = signal<number>(99.96);

  protected readonly uptimeText = computed(() => this.formatDuration(this.status()?.uptimeSeconds));
  ...
  protected readonly sparklinePercents = computed(() => {
  ...
  protected readonly throughputChartData = computed<ChartData<'line'>>(() => ({
  ...
  protected readonly modelShareData = computed<ChartData<'doughnut'>>(() => ({
  ...
  protected readonly latencyChartData = computed<ChartData<'bar'>>(() => ({
  ...
  protected readonly throughputChartOptions: ChartOptions<'line'> = {
    responsive: true,
  ...
  protected readonly modelShareOptions: ChartOptions<'doughnut'> = {
    cutout: '65%',
    plugins: {
  ...
  protected readonly latencyChartOptions: ChartOptions<'bar'> = {
    indexAxis: 'y',
    responsive: true,
    maintainAspectRatio: false,
    plugins: {
  ...
  protected statusSeverity(status: BernardStatus | null) {
    if (!status) {
      return 'secondary';
    }
