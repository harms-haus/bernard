...
export default function useInterruptedActions({
  ...
  const handleSubmit = async (
    ...
    e.preventDefault();
    if (!humanResponse || humanResponse.length === 0) {
      toast.error("Error", {
    ...
    if (
      ...
      setLoading(true);
      try {
        const resumedSuccessfully = resumeRun(humanResponse);
        if (resumedSuccessfully) {
          toast("Success", {
            description: "Response submitted successfully.",
            duration: 5000,
  ...
  const handleIgnore = async (
    ...
    try {
      const resumedSuccessfully = await Promise.resolve(resumeRun([ignoreResponse]));
      if (resumedSuccessfully) {
