    ...
    <div className="flex flex-col gap-4 items-start w-full p-6 rounded-lg border-[1px] border-gray-300">
      ...
      {Object.entries(editResponse.args.args).map(([k, v], idx) => {
        ...
        if (
          ...
          defaultRows.current[k as keyof typeof defaultRows.current] = !valueLength
            ? 3
            : Math.max(valueLength / 30, 7);
        }
        const numRows =
...
export function InboxItemInput({
  ...
  const onEditChange = (
    ...
    setHumanResponse((prev) => {
      ...
      if (matchingResponse) {
        ...
        console.error("No matching response found");
        toast.error("Error", {
          description: "No matching response found",
          richColors: true,
          closeButton: true,
        });
        return prev;
  ...
  const onResponseChange = (
    ...
    setHumanResponse((prev) => {
      ...
      if (!matchingResponse) {
        console.error("No human response found for string response");
        toast.error("Error", {
          description: "No human response found for string response",
          richColors: true,
          closeButton: true,
        });
        return prev;
