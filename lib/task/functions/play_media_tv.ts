import type { TaskExecutionContext, TaskResult } from "../types";
import { exec } from "child_process";
import { promisify } from "util";
import { getHAConnection } from "../agent/harness/router/tools/utility/home-assistant-websocket-client";
import { getEntityState } from "../agent/harness/router/tools/home-assistant-get-entity-state.tool";
import {
  resolveDeviceConfig,
  resolveHAEntityId,
...
} from "../agent/harness/router/tools/utility/plex-device-mapping";
 ...
 */
function rankSearchResults(results: PlexMediaItem[], query: string): PlexMediaItem[] {
 ...
 */
async function searchPlexBestMatch(
  plexConfig: PlexConfig,
  mediaQuery: string,
...
async function ensureTvOn(
  ...
  try {
    ...
    const state = await getEntityState(haRestConfig, haEntityId);
    if (state?.state === 'on') {
...
export async function playMediaTvTask(
  ...
  const { location_id, media_query, playback_mode = "resume" } = args;
